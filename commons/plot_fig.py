...
def plot_fig(agent):
    ...
    prob = np.array(agent.accept_prob)
    atime = np.array(agent.accept_time)
    state_meas = np.array(agent.xx)
    nom_state = np.array(agent.zh)
    ...
    plt.savefig('figs/risk.svg', bbox_inches='tight', pad_inches=0.1, transparent=True)
    ...
    # Save figure
    plt.savefig('figs/map.svg', bbox_inches='tight', pad_inches=0.1, transparent=True)
